...
function SignUp({ onLinkClickHandler }: Props) {
    ...
    //                    event handler                    //
    // change 핸들러는 값을 입력하는 모든 상황
    // button 핸들러는 중복된 값같이 버튼으로 확인하는 상황 

     // 아이디 관련 상태 및 메시지를 업데이트
    const onIdChangeHandler = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    // 아이디 중복 확인 버튼 클릭 / 아이디 중복 확인 API 요청
    const onIdButtonClickHandler = () => {
        if(!idButtonStatus) return;
        // 아이디가 빈 문자열이나 공백이면 리턴 
    ...
    // 인증번호 확인 버튼을 클릭 / 이메일 인증번호 확인 API요청
    ...
    // 회원가입 버튼을 클릭했을 때 호출 / 회원가입 API 요청
